function [alpha, beta, u_c, sigma_c, lambda_c, u_i1, sigma_i1, lambda_i1] = EM2_3(S,sl1,sl2)
...
%     ll = sum( sum(log(P_i), 1) + log(alpha * sum(P_c ./ P_i, 1) + (1-alpha) * N) );
    ...
    disp(ll);
    disp(ll - prev_ll);
    disp([alpha, beta, u_c, sigma_c, lambda_c, u_i1, sigma_i1, lambda_i1]);
...
%     Wi2 = Wi(2,:);
    ...
    [Vi2, Wi2] = trunc_norm_moments(delta_i2 / sigma_i2 * (s2-u_i2), sqrt(1-delta_i2^2));
   
%     S1 = S(1,:);
%     S2 = S(2,:);

    Rs1 = rs(s1, alpha, u_i1, sigma_i1, lambda_i1, u_c, sigma_c, lambda_c);
    Rs2 = rs(s2, beta, u_i2, sigma_i2, lambda_i2, u_c, sigma_c, lambda_c);
    sum_Rs1 = sum(Rs1);
    sum_Rs2 = sum(Rs2);
    ...
    u_i2_new = (sum( (1 - Rs2) .* (s2 - Vi2 * Delta_i2) )) / (M2 - sum_Rs2);
...
%     ll = func_ll2(S, alpha, u_c_new, Delta_c, Gamma_c, u_i_new, Delta_i, Gamma_i)
    ...
    Delta_i2_new = (sum((1 - Rs2) .* Vi2 .* (s2 - u_i2_new))) / (sum((1 - Rs2) .* Wi2));
...
%     ll = func_ll2(S, alpha, u_c_new, Delta_c_new, Gamma_c, u_i_new, Delta_i_new, Gamma_i)
    ...
    Gamma_i1_new = ( sum( (1-Rs1) .* ((s1 - u_i1_new).^2 - 2 * Vi1 .* (s1 - u_i1_new) * Delta_i1_new + Wi1 * Delta_i1_new^2) ) ) ...
        / (M1 - sum_Rs1);
    Gamma_i2_new = ( sum( (1-Rs2) .* ((s2 - u_i2_new).^2 - 2 * Vi2 .* (s2 - u_i2_new) * Delta_i2_new + Wi2 * Delta_i2_new^2) ) ) ...
        / (M2 - sum_Rs2);
