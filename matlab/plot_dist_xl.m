...
yic = skew_norm_pdf(S1, u_ic, sigma_ic, lambda_ic);
...
l_pdf_i1 = plot(S1,yi1*beta,'Color',i1_color,'LineWidth',linewidth);
l_pdf_ic = plot(S1,yic*(1-alpha-beta),'Color',ic_color,'LineWidth',linewidth);
...
hic = skew_norm_cdf(S1, u_ic, sigma_ic, lambda_ic);
h1 = alpha*hc + beta*hi1 + (1-alpha-beta)*hic;
...
if plotcdf
    ...
    text(xlim_high-2*bin_width, 0.8, ...
    ...
    if legending
        legending = false;
        legend([l_hist_s1, l_pdf_c, l_pdf_i1, l_pdf_ic, l_pdf_s1, l_cdf_emp, l_cdf_est],...
            {'top score'; 'correct'; 'incorrect-incorrect'; 'incorrect-correct'; 'mixture'; 'empirical CDF'; 'estimated CDF'});
    end
else
    if legending
        legending = false;
        legend([l_hist_s1, l_pdf_c, l_pdf_i1, l_pdf_ic, l_pdf_s1],...
            {'top score'; 'correct'; 'incorrect-incorrect'; 'incorrect-correct'; 'mixture'});
    end
end
