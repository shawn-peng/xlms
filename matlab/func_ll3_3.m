...
function ll = func_ll3_3(S1, S2, S3, alpha, beta, u_c, sigma_c, lambda_c, u_i1, sigma_i1, lambda_i1, u_i2, sigma_i2, lambda_i2, u_i3, sigma_i3, lambda_i3)
    ...
    p = alpha * P_c1 .* P_i21 .* P_i32 + (1 - alpha)*beta * P_i1 .* P_c2 .* P_i32 ...
        + (1 - alpha)*(1 - beta) * P_i1 .* P_i22 .* P_i33;
    
%     p1 = alpha * P_c1 + (1 - alpha) * P_i1;
%     p2 = (1 - alpha)*beta * P_c2 + alpha * P_i21 + (1 - alpha)*(1 - beta) * P_i22;
%     p3 = (alpha + (1 - alpha)*beta) * P_i32 + (1 - alpha)*(1 - beta) * P_i33;
    
    flags = p==0;
%     disp([sum(flags), sum(flags) .* log(min(p(~flags)))])
