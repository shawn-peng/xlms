function [alpha, beta, u_c, sigma_c, lambda_c, u_i, sigma_i, lambda_i] = EM2(S,sl1,sl2)
...
%     ll = sum( sum(log(P_i), 1) + log(alpha * sum(P_c ./ P_i, 1) + (1-alpha) * N) );
    ll = func_ll(S, alpha, beta, u_c, sigma_c, lambda_c, u_i, sigma_i, lambda_i);
    disp(ll);
    disp(ll - prev_ll);
    ...
    beta_new = 1 / M * sum(sum(Rs2));

    u_i_new = sum(sum( (1 - Rs1) .* (S1 - Vi1 * Delta_i) + (1 - Rs2) .* (S2 - Vi2*Delta_i) )) / (2*M - sum_Rs1 - sum_Rs2);
    u_c_new = sum(sum( Rs1 .* (S1 - Vc1 * Delta_c) + Rs2 .* (S2 - Vc2 * Delta_c) )) / (sum_Rs1+sum_Rs2);
%     ll = func_ll(S, alpha, u_c_new, Delta_c, Gamma_c, u_i_new, Delta_i, Gamma_i)


    Delta_i_new = sum(sum((1 - Rs1) .* Vi1 .* (S1 - u_i_new) + (1 - Rs2) .* Vi2 .* (S2 - u_i_new))) / sum(sum((1 - Rs1) .* Wi1 + (1 - Rs2) .* Wi2));
    Delta_c_new = sum(sum(Rs1 .* Vc1 .* (S1 - u_c_new) + Rs2 .* Vc2 .* (S2 - u_c_new))) / sum(sum(Rs1 .* Wc1 + Rs2 .* Wc2));
...
%     ll = func_ll(S, alpha, u_c_new, Delta_c_new, Gamma_c, u_i_new, Delta_i_new, Gamma_i)
    ...
    Gamma_i_new = sum(sum( (1-Rs1) .* ((S1 - u_i_new).^2 - 2 * Vi1 .* (S1 - u_i_new) * Delta_i_new + Wi1 * Delta_i_new^2) ...
        + (1-Rs2) .* ((S2 - u_i_new).^2 - 2 * Vi2 .* (S2 - u_i_new) * Delta_i_new + Wi2 * Delta_i_new^2) )) / (2*M - sum_Rs1 - sum_Rs2);
    Gamma_c_new = sum(sum( Rs1 .* ((S1 - u_c_new).^2 - 2 * Vc1 .* (S1 - u_c_new) * Delta_c_new + Wc1 * Delta_c_new^2) ...
        + Rs2 .* ((S2 - u_c_new).^2 - 2 * Vc2 .* (S2 - u_c_new) * Delta_c_new + Wc2 * Delta_c_new^2) )) / (sum_Rs1 + sum_Rs2);
